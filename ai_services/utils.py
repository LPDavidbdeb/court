...
class EvidenceFormatter:
    ...
    def _get_protagonist_display(protagonist, fallback_name):
        """
        Returns 'Name [Role]' if a protagonist is linked, otherwise the fallback name.
        Ex: 'Marie-Josée Ayoub [Avocate en droit de la famille]'
        """
        if not protagonist:
    ...
    def format_photo_document_text(photo_doc):
        """Returns the text context for a photo document."""
    ...
    def unpack_narrative_multimodal(cls, narrative):
        """
        Returns a LIST of objects [str, PIL.Image, str, ...] for the Gemini API.
        Interleaves text and images in strict chronological order.
        """
        timeline = []
